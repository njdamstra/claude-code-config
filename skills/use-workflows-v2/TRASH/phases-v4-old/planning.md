...
Create implementation steps based on discovery findings.

## Step 1: Review Discovery Outputs

Before planning, review all discovery phase deliverables from `{{phase_paths.discovery}}/`.

Use Read tool to examine each file and understand:
- What patterns exist in the codebase
- What files are relevant
- What constraints or conventions to follow
...
```
Planning phase complete with gaps:
...
If user selects retry: Return to Step 2 with refined approach.
...
If user selects "Finalize": Proceed to finalization.
If user selects "Revise": Return to Step 2.
If user selects "Abort": Stop workflow.
{{/checkpoint}}
...
Continue to workflow finalization.
{{/checkpoint}}
