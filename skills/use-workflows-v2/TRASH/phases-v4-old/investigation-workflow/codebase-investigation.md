...
**Note:** This is a workflow-specific override for the 'investigation-workflow'.

Comprehensive codebase research using specialized subagent to identify patterns, dependencies, and integration points. This version adds extra guidance for the investigation.
...
Task(
  ...
  prompt="Research codebase for patterns, dependencies, and integration points related to {{feature_name}}. Focus on {{scope}} scope. Identify reusable components, similar implementations, and architectural patterns. Output to {{workspace}}/{{output_dir}}/{{configured_filename}}"
)
```

**Expected Output:** `{{workspace}}/{{output_dir}}/{{configured_filename}}`
