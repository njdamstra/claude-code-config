...
useSpring is a simpler hook than [useMotion](https://motion.vueuse.org/api/use-motion) and makes it possible to animate HTML or SVG Elements in your Vue components using spring animations.
...
It skips the [Variants](https://motion.vueuse.org/features/variants) system, allowing it to be as performant as using Popmotion natively, but with a nicer **API** to play with Vue refs.

## Parameters

### `values`

The values argument expects a `motionProperties` object, which can be created using the [useMotionProperties](https://motion.vueuse.org/api/use-motion-properties) function.
